...
var _getPar = function(par) {
	...
	var ret = "";
	document.location.href.split("?")[1].split("&").forEach(function(kv) {
		var es = kv.split("=");
		if (es[0] == par) {
			ret = decodeURIComponent(es[1]);
		}
	});
	return ret;
};
...
var _getScript = function(src, func) {
...
var _hash_copy = function(s, d) {
	Object.keys(s).forEach(function(k) {
		d[k] = s[k];
	});
};

var _shrink_list = function(m, k) {
	var ret = {};
